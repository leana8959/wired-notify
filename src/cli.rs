...
impl CLIListener {
    ...
    // Should be spawn on a second thread. Loops forever to handle incoming requests.
    pub fn handle(self: &Self, sender: EventLoopProxy<NotifyEvent>) {
        let handle_one_connection = |stream: Result<UnixStream, io::Error>| -> () {
            ...
            for line in reader.lines() {
                let line = match line {
                    Err(e) => {
                        eprintln!("Error reading a line of command from the socket: {:?}", e);
                        continue;
                    }
                    Ok(l) => l,
                };
        ...
        self.listener.incoming().for_each(handle_one_connection);
    }
}
...
pub fn handle_cli_command(
    ...
    let (command, args) = match command.split_once(':') {
        Some((x, xs)) => (x, xs),
        None => {
            eprintln!("Internal error: The command `{}` is malformed.", command);
        ...
        if matches.opt_present("Z") {
            writeln!(sock, "dnd-status:") // read_line blocks until newline, hence *ln;
                ...
                .and_then(|_| {
                    ...
                    println!("dnd:{}", if buf.trim() == "true" { "on" } else { "off" });
                    Ok(())
                })
                .map_err(|e| e.to_string())?;
        }
